...
bool write_inode_bitmap() {
        ...
        chunk = 0x000000FF;     // reserve inode 0 for invalid inode, inode 1 for root dir, inode 2 for prekernel, 3 for kernel
...
bool write_data_bitmap() {
        ...
        chunk = 0x01FFFFFF;     // reserve data blocks for prekernel & kernel (hardcoded b/c im a coward)
        count = fwrite(&chunk, 4, 1, disk_ptr);
        if(count != 1) {
...
bool write_file_data(char* dir_path, uint32_t curr_inode, uint32_t parent_inode) {
        ...
        while(dir_ent) {
                ...
                if(dir_ent->d_type == DT_REG) {
                        ...
                        uint32_t temp = 0;
                        if(first_block <= 13 && first_block >= 7) {
                                if(strncmp(dir_ent->d_name, "kernel.bin", 11) != 0) {
                                        first_block+=7; // this conditional will first occur when first_block = 7
                                }
                        }
                        if(strncmp(dir_ent->d_name, "kernel.bin", 11) == 0) {
                                temp = first_block;
